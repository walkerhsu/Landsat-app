...
const CountdownTimer = () => {
  ...
  if (person.locationHistory.length === 0 || !isSignedIn) {
    return (
        <div className="text-xl font-mono mb-4 text-white flex-wrap" style={
            {textAlign: 'center'}
          }>Sign in and add location to view count down timer</div>
  ...
  return (
    <>
      <div className="text-l font-mono text-white flex-wrap" style={
        {textAlign: 'center'}
      }>
        Landsat {landsatID} is going to pass over {location!} in:
      </div>
      <div className="text-3xl font-mono text-red-400">
        {timeLeft.days}{timeLeft.hours}h-{timeLeft.minutes}m-{timeLeft.seconds}s
      </div>
    </>
  );
};
